...
class GcodeVM {
	...
	init() {
		...
		vmState.axisLabels = this.options.axisLabels || controller.axisLabels || [ 'x', 'y', 'z' ];
		...
		vmState.feed = controller.feed || this.options.maxFeed || 1000;
	...
	_processMove(to, axisFlags, feed = null, travel = null, incremental = false) {
		...
		if (this.vmState.inverseFeed) {
			...
			let accel = this.options.acceleration || 36000; // in mm / min^2
			for (let axisNum = 0; axisNum < to.length; axisNum++) {
