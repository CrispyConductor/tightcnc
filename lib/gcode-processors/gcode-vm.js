...
class GcodeVM extends GcodeProcessor {
	...
	initProcessor() {
		...
		let controller = this.processorOptions.controller || (this.processorOptions.tightcnc && this.processorOptions.tightcnc.controller) || {};
		let vmState = {};
		...
		vmState.pos = (controller.getPos && controller.getPos()) || controller.pos || this.zerocoord();
	...
	getStatus() {
		...
		return {
			units: this.vmState.units,
			line: this.vmState.line,
			totalTime: this.vmState.totalTime,
			lineCounter: this.vmState.lineCounter,
			bounds: this.vmState.bounds
		};
	}
